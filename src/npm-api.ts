...
import { PackageDownloads, PackageInfo, GitHubStats, UserPackageStats, NPMSearchResult, PackageStatsWithRanking } from './types.js';
...
export class NPMStatsAPI {
  private readonly baseURL = 'https://api.npmjs.org';
   ...
   */
  async getPackageDownloads(packageName: string, period = 'last-month'): Promise<PackageDownloads> {
    ...
    return response.json() as Promise<PackageDownloads>;
  }
   ...
   */
  private extractGitHubRepo(packageInfo: PackageInfo): { owner: string; repo: string } | null {
   ...
   */
  async getGitHubStats(owner: string, repo: string, token?: string): Promise<GitHubStats> {
   ...
   */
  async searchPackagesByAuthor(username: string): Promise<string[]> {
   ...
   */
  async getUserPackageStats(username: string, includeGitHub = false, githubToken?: string): Promise<UserPackageStats> {
    ...
    const packageStats: PackageStatsWithRanking[] = [];
    const packagesWithoutStats: string[] = []; // Track packages without download stats
    ...
    sortedByMonthly.forEach((pkg, index) => {
      ...
      if (originalPkg) originalPkg.monthlyRank = index + 1;
    });
    ...
    sortedByWeekly.forEach((pkg, index) => {
      const originalPkg = successfulStats.find(p => p.name === pkg.name);
      if (originalPkg) originalPkg.weeklyRank = index + 1;
    });
   ...
   */
  async getPackageDownloadHistory(packageName: string): Promise<{
